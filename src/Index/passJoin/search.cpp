void Search::build(SharedInfo<IDType> *sharedInfo)
{
...
void Search::search(const string &query, Similarity similarity, vector<int> *ids) {
}

void Search::insertED(int ed, IDType id, const char *record)
{
	edIndexes[ed].insert(mapping.insert(id, record), record);
}

void Search::insertHM(int hm, IDType id, const char *record)
{
	hmIndexes[hm].insert(mapping.insert(id, record), record);
}

void Search::searchHM(int hm, const char *query, std::vector<IDType> &ids, std::vector<int> &hms)
{
	D = hm;
...
void Search::searchED(int ed, const char *query, std::vector<IDType> &ids, std::vector<int> &eds)
{
	D = ed;
